...
{
	if (r == 1)	return "(GloBot) Mephestys";		// Of course I have to join in first :)
	else if (r == 2)	return "(GloBot) FrikaC";
	else if (r == 3)	return "(GloBot) KrimZon";
	else if (r == 4)	return "(GloBot) Akuma";
	else if (r == 5)	return "(GloBot) scar3crow";
	else if (r == 6)	return "(GloBot) Quest";
	else if (r == 7)	return "(GloBot) CocoT";
	else if (r == 8)	return "(GloBot) LordHavoc";
	else if (r == 9)	return "(GloBot) KyleMac";
	else if (r == 10)	return "(GloBot) Koolio";
	else if (r == 11)	return "(GloBot) Krust";
	else if (r == 12)	return "(GloBot) BramBo";
	else if (r == 13)	return "(GloBot) Horn";
	else if (r == 14)	return "(GloBot) c0burn";
	else if (r == 15)	return "(GloBot) Heffo";
...
void BotConnect(client_t *client, int ClientNo, int color, char *name)
{
	...
	bot->v.netname = PR_SetEngineString(client->name);		// Everyone wants a name
...
{
	if (!sv.active) {
		Con_SafePrintf("Not hosting a multiplayer server.\n");
		return;
	}
